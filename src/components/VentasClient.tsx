...
const ORDER_STATUS_NEXT: Record<OrderStatus, OrderStatus> = {
  pendiente: 'verificado', verificado: 'enviado', enviado: 'pendiente',
}
...
export default function VentasClient({ initialOrders }: Props) {
  ...
  function exportCSV() {
    ...
    const rows = filtered.map(o => [o.name, o.phone, o.address, o.quantity.toString(), o.status, new Date(o.created_at).toLocaleDateString('es-AR')])
  ...
  const filtered = orders.filter(o => filter === 'all' ? true : o.status === filter).filter(o => { if (!search) return true; const q = search.toLowerCase(); return o.name.toLowerCase().includes(q) || o.phone.includes(q) })
  const counts: Record<string, number> = {}
  for (const f of FILTERS) counts[f.key] = f.key === 'all' ? orders.length : orders.filter(o => o.status === f.key).length
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '16px 24px', borderBottom: '1px solid #1e2d45', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
        <div>
          ...
          <p style={{ fontSize: '12px', color: '#64748b', marginTop: '2px' }}>{orders.reduce((a, o) => a + o.quantity, 0)} unidades en total</p>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <div style={{ position: 'relative' }}>
            <Search size={14} style={{ position: 'absolute', left: '12px', top: '50%', transform: 'translateY(-50%)', color: '#64748b' }} />
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar orden..." style={{ paddingLeft: '36px', paddingRight: '16px', paddingTop: '8px', paddingBottom: '8px', background: '#1e293b', border: '1px solid #1e2d45', borderRadius: '8px', color: '#f8fafc', fontSize: '13px', outline: 'none', width: '240px' }} />
          </div>
      ...
      <div style={{ display: 'flex', gap: '4px', padding: '12px 24px', borderBottom: '1px solid #1e2d45', flexShrink: 0 }}>
        {FILTERS.map(f => {
          ...
          return (
            <button key={f.key} onClick={() => setFilter(f.key)} style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '6px 14px', borderRadius: '7px', fontSize: '13px', fontWeight: 500, cursor: 'pointer', transition: 'all 0.15s', background: isActive ? 'rgba(249,115,22,0.12)' : 'transparent', border: isActive ? '1px solid rgba(249,115,22,0.35)' : '1px solid transparent', color: isActive ? '#f97316' : '#64748b' }}>
              {f.label}
              {counts[f.key] > 0 && <span style={{ background: isActive ? '#f97316' : '#1e293b', color: isActive ? '#fff' : '#64748b', borderRadius: '999px', padding: '1px 7px', fontSize: '11px', fontWeight: 700 }}>{counts[f.key]}</span>}
            </button>
          )
        })}
      </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {filtered.map(order => {
                ...
                return (
                  <tr key={order.id} style={{ borderBottom: '1px solid #1e2d45', transition: 'background 0.1s' }} onMouseEnter={e => (e.currentTarget as HTMLTableRowElement).style.background = '#1e293b'} onMouseLeave={e => (e.currentTarget as HTMLTableRowElement).style.background = 'transparent'}>
                    ...
                    <td style={{ padding: '12px', fontSize: '13px', color: '#94a3b8' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><Phone size={11} color="#64748b" />{order.phone}</div>
                    </td>
                    <td style={{ padding: '12px', fontSize: '13px', color: '#64748b', maxWidth: '200px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><MapPin size={11} color="#475569" />{order.address}</div>
                    </td>
                    <td style={{ padding: '12px', fontSize: '13px', fontWeight: 700, color: '#f97316' }}>{order.quantity} uds</td>
                    <td style={{ padding: '12px' }}>
                      <button onClick={() => handleStatusToggle(order)} disabled={isPending} style={{ padding: '4px 10px', borderRadius: '6px', fontSize: '12px', fontWeight: 600, background: cfg.bg, color: cfg.color, border: `1px solid ${cfg.color}40`, cursor: 'pointer' }}>
                        {cfg.label} →
                      </button>
                    </td>
                    <td style={{ padding: '12px', fontSize: '12px', color: '#475569' }}>{new Date(order.created_at).toLocaleDateString('es-AR')}</td>
                  </tr>
                )
              })}
            </tbody>
