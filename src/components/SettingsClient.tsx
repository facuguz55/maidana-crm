...
import { CheckCircle, XCircle, RefreshCw, Zap, MessageSquare, Database, Eye, EyeOff } from 'lucide-react'
...
export default function SettingsClient({ settings }: Props) {
  ...
  async function handleTest() {
    setTestResult(null)
    try {
      const res = await fetch('/api/test-evolution', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ url: form.evolution_api_url, apiKey: form.evolution_api_key }) })
      ...
      <div className="settings-content" style={{ padding: '24px 32px', maxWidth: '720px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        <Card icon={Zap} iconColor="#f97316" title="Evolution API" subtitle="Configuración de WhatsApp">
          ...
          <div style={{ display: 'flex', gap: '10px', marginTop: '4px' }}>
            <button onClick={handleTest} style={{ flex: 1, padding: '9px', borderRadius: '8px', fontSize: '13px', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', background: testResult === 'ok' ? 'rgba(34,197,94,0.12)' : testResult === 'error' ? 'rgba(239,68,68,0.12)' : 'rgba(59,130,246,0.12)', border: `1px solid ${testResult === 'ok' ? '#22c55e40' : testResult === 'error' ? '#ef444440' : '#3b82f640'}`, color: testResult === 'ok' ? '#22c55e' : testResult === 'error' ? '#ef4444' : '#3b82f6' }}>
              {testResult === 'ok' ? <><CheckCircle size={14} /> Conectado</> : testResult === 'error' ? <><XCircle size={14} /> Error</> : 'Probar conexión'}
            </button>
            <button onClick={handleSave} disabled={isPending} style={{ flex: 1, padding: '9px', background: 'rgba(249,115,22,0.12)', border: '1px solid rgba(249,115,22,0.3)', borderRadius: '8px', color: '#f97316', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}>
        ...
        <Card icon={MessageSquare} iconColor="#3b82f6" title="Mensaje del Formulario" subtitle="Texto enviado al cliente con el link">
          <div>
            ...
            <textarea value={form.form_message} onChange={e => update('form_message', e.target.value)} placeholder="Hola! Para completar tu pedido completá este formulario: https://forms.gle/..." rows={3} style={{ width: '100%', padding: '10px 14px', background: '#0d1526', border: '1px solid #1e2d45', borderRadius: '8px', color: '#f8fafc', fontSize: '13px', resize: 'vertical', outline: 'none', fontFamily: 'inherit', lineHeight: 1.5 }} />
          </div>
          <button onClick={handleSave} disabled={isPending} style={{ padding: '8px 20px', background: 'rgba(249,115,22,0.12)', border: '1px solid rgba(249,115,22,0.3)', borderRadius: '8px', color: '#f97316', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}>
